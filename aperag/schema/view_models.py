...
#   timestamp: 2025-09-20T03:43:24+00:00
...
class Agent1(BaseModel):
    ...
    system_prompt_template: Optional[str] = None
    query_prompt_template: Optional[str] = None
    collections: Optional[list[SharedCollection]] = None


class SharedBotConfig(BaseModel):
    """
    Configuration settings for shared bot (read-only version)
    """

    agent: Optional[Agent1] = None
    flow: Optional[WorkflowDefinition] = None


class SharedBot(BaseModel):
    """
    Shared bot information for marketplace users
    """

    id: str = Field(..., description='Bot ID')
    title: str = Field(..., description='Bot title')
    description: Optional[str] = Field(None, description='Bot description')
    type: Literal['knowledge', 'common', 'agent'] = Field(..., description='Bot type')
    owner_user_id: str = Field(..., description='Original owner user ID')
    owner_username: Optional[str] = Field(None, description='Original owner username')
    subscription_id: Optional[str] = Field(
        None,
...
class SharedBotList(BaseModel):
    """
    Shared bot list response
    """

    items: list[SharedBot] = Field(..., description='List of shared bots')
    total: int = Field(..., description='Total count (for pagination)')
    page: int = Field(..., description='Current page number')
    page_size: int = Field(..., description='Page size')
